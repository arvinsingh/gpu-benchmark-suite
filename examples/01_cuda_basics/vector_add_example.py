...
def demonstrate_cuda_basics():
    ...
    print(f"   PyTorch GPU:   {pytorch_time:8.2f} ms (speedup: {cpu_time/pytorch_time:.1f}x)")
    if CUDA_AVAILABLE:
        print(f"   CUDA kernel:   {cuda_time:8.2f} ms (speedup: {cpu_time/cuda_time:.1f}x)")
...
def demonstrate_thread_organization():
    ...
    for N in problem_sizes:
        ...
        print(f"   N = {N:>8,}: {num_blocks:>4} blocks × {block_size} threads = {total_threads:>8,} total threads")
        print(f"                Efficiency: {efficiency:.1%} (active threads / total threads)")

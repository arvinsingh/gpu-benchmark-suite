...
app = typer.Typer(help="GPU Benchmark Suite - Performance testing for CUDA, Triton, and PyTorch")
console = Console()
...
@app.command()
def list_benchmarks(
    category: Optional[str] = typer.Option(None, "--category", "-c", help="Benchmark category to list")
) -> None:
...
def run(
    ...
    benchmark: Optional[str] = typer.Option(None, "--benchmark", "-b", help="Specific benchmark to run"),
    backends: List[str] = typer.Option(["cuda", "triton", "pytorch"], "--backend", help="Backends to test"),
    sizes: List[int] = typer.Option([1024, 4096, 16384], "--size", help="Problem sizes to test"),
    device_id: int = typer.Option(0, "--device", "-d", help="GPU device ID"),
    ...
    try:
        ...
        if benchmark:
            ...
            results = runner.run_benchmark(
                category, benchmark, backends, sizes, 
                warmup_runs=warmup, benchmark_runs=runs
            )
        else:
...
@app.command()
def compare(
    benchmark: str = typer.Argument(..., help="Benchmark to compare (format: category/name)"),
    backends: List[str] = typer.Option(["cuda", "triton", "pytorch"], "--backend", help="Backends to compare"),
    sizes: List[int] = typer.Option([1024, 4096, 16384], "--size", help="Problem sizes to test"),
    device_id: int = typer.Option(0, "--device", "-d", help="GPU device ID"),
    baseline: str = typer.Option("pytorch", "--baseline", help="Baseline backend for comparison"),
) -> None:
    """Compare different implementations of a specific benchmark."""
    try:
        if "/" not in benchmark:
            rprint("[red]Benchmark must be in format category/name (e.g., memory/vector-add)[/red]")
            return
...
@app.command()
def profile(
    benchmark: str = typer.Argument(..., help="Benchmark to profile (format: category/name)"),
    backend: str = typer.Argument(..., help="Backend to profile (cuda, triton, pytorch)"),
...
def _print_comparison_table(comparison: dict) -> None:
    ...
    for bench_name, backends in comparison.items():
        for backend_name, metrics in backends.items():
            time_ms = f"{metrics['execution_time']:.3f}" if metrics['execution_time'] else "N/A"
